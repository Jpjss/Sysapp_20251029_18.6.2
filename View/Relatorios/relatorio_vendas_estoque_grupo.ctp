...
<table id="cabecalhoTabela" width="100%" border="0" cellspacing="0" cellpadding="0" style="text-align: center; border-color:red">
    <tr>
        <td style="font-size: 24px;">Relatório Vendas/Estoque por Grupo -  <?php echo $this->Session->read('Conexao.Ativa'); ?><br></td>
    </tr>
    <tr>
...
<br>
  	<input type="button" class="btn btn-primary" id="voltar" value="Voltar" onClick="javascript:navigator_Go('/SysApp/app/webroot/index.php/Relatorios/vendas_estoque_grupo')"/>
<br>
<div class="contentRelatorio">
    ...
    ?>
       	<tr>
        	<td></td>
            <td><?php echo $value['ds_grupo']; ?></td>
            <td><?php echo number_format($value['qtde_estoque_real'],0, ',' , '.'); ?></td>
            <td><?php echo number_format($value['perc_qtde_estoque'],2, ',' , '.'); ?></td>
            <td><?php echo number_format($value['qtde_venda'],0, ',' , '.'); ?></td>
            
            <?php if($chave['total_qtde_venda'] != 0){?>
            <td><?php echo number_format(($value['qtde_venda']/$chave['total_qtde_venda'])*100, 2, ',', '.')?></td>
            <?php }else{?>
            <td><?php echo number_format(0, 2, ',', '.')?></td>
            <?php }?>
            
            <td><?php echo number_format($value['vlr_estoque'],2, ',' , '.'); ?></td>
            <?php if($chave['total_vlr_estoque'] != 0){?>
            <td><?php echo number_format(($value['vlr_estoque']/$chave['total_vlr_estoque'])*100, 2, ',', '.')?></td>
            <?php }else{?>
            <td><?php echo number_format(0, 2, ',', '.')?></td>
            <?php }?>
            <td><?php echo number_format($value['vlr_venda'],2, ',' , '.'); ?></td>
            <td><?php echo number_format(($value['vlr_venda']/$chave['total_vlr_venda'])*100 ,2, ',' , '.'); ?></td>
            
            <!--  Rela��o R$  -->
            <?php if(($value['vlr_venda']*$value['pcusto']) != 0){?>
            <td><?php echo number_format(($value['vlr_estoque']/($value['vlr_venda'])),2, ',' , '.'); ?></td>
            <?php }else{?>
            <td><?php echo number_format(0,2, ',' , '.'); ?></td>
            <?php }?>
        ...
        <tr style="text-align: center; background: #59a6d6; border: 0px;">
            ...
            <td>Vlr. Vendido</td>
            <td>% Vlr. Venda</td>
            <td>Rela&ccedil;&atilde;o R$</td>
            <td>Rela&ccedil;&atilde;o Estoque</td>
        ...
        <tr style="text-align: center; background: #32C7FF; border: 0px;">
            ...
            <td><?php echo number_format($chave['total_perc_estoque'], 2, ',', '.'); ?></td>
            <td><?php echo number_format($chave['total_qtde_venda'],0, ',' , '.'); ?></td>
            <td><?php echo number_format(($chave['total_qtde_venda']/$totalQtdeVenda)*100, 2, ',', '.'); ?></td>
            <td><?php echo number_format($chave['total_vlr_estoque'],2, ',' , '.'); ?></td>
            <?php if($totalVlrEstoque != 0){?>
            <td><?php echo number_format(($chave['total_vlr_estoque']/$totalVlrEstoque)*100, 2, ',', '.'); ?></td>
            <?php }else{?>
            <td><?php echo number_format(0, 2, ',', '.'); ?></td>
            <?php }?>
			<td><?php echo number_format($chave['total_vlr_venda'],2, ',' , '.'); ?></td>
			<td><?php echo number_format(($chave['total_vlr_venda']/$totalVlrVenda)*100, 2, ',', '.'); ?></td><?php  if($chave['qtde_p_custo'] != 0){$totalGeralPCusto = $chave['total_p_custo'] / $chave['qtde_p_custo'];}else{$totalGeralPCusto = 0;} ?>
			<?php if(($chave['total_qtde_venda']*($totalGeralPCusto) != 0)){?>
			<td><?php echo number_format(($chave['total_vlr_estoque']/($chave['total_qtde_venda']*($totalGeralPCusto))), 2, ',', '.'); ?></td>
			<?php }else{?>
			<td><?php echo number_format(0, 2, ',', '.'); ?></td>
			<?php }?>
        ...
        <tr>
            ...
            <!--  Rela��o R$  -->
            <?php if(($value['vlr_venda']*$value['pcusto']) != 0){?>
            <td><?php echo number_format(($value['vlr_estoque']/($value['vlr_venda']*$value['pcusto'])),2, ',' , '.'); ?></td>
            <?php }else{?>
            <td><?php echo number_format(0,2, ',' , '.'); ?></td>
            <?php }?>
