...
<script>
    $(document).ready(function () {
        ...
        $("#marcaTodosPagamentos").change(function () {
            $(".pagamentos").prop('checked', $(this).prop("checked"));
        });
        ...
        $('#enviar').click(function () {
            ...
            var booPagamentoMarcado = getCheckedBoxes("data[Relatorios][pagamentos][]");
            ...
            if (booPagamentoMarcado === false) {
                $('.mensagem').html("<div class='alert alert-danger fade in' role='alert' size='2'><button type='button' class='close' data-dismiss='alert' aria-label='Fechar'><span aria-hidden='true'>&times;</span></button><strong>Nenhum Tipo de Pagamento Selecionado!</strong></div>");
                div$(".mensagem").fadeTo(3000, 500).slideUp(500, function () {
                    $(".mensagem").alert('close');
                });
                return false;
            }
...
<style>
    ...
    .container3 { border:2px solid #ccc; width:auto; height: 300px; overflow-y: auto;} /* div filiais*/
    .container4 { border:2px solid #ccc; width:auto; height: auto; overflow:hidden; vertical-align: middle; }
    .container5 { border:2px solid #ccc; width:auto; height: auto; overflow:hidden; vertical-align: middle; } /* tipo de agrupamento */
...
<div class="vendasVendedor">
    <h2><?php echo __('Relatório de Vendas por Condição de Pagamento'); ?></h2>
    ...
    <table class="tabelaPrincipal">
            ...
            <td>
                ...
                <div class="container4">
                    <table id="tabelaPedPendente" >
                        <td><input name="data[Relatorios][per_ini_vendas]" value="<?php echo date("01/m/Y"/* ,strtotime("-1 month") */) ?>" id="per_ini_vendas" type="text"><br> a <br></b><input name="data[Relatorios][per_fim_vendas]" value="<?php echo date("d/m/Y"); ?>" id="per_fim_vendas" type="text"></td>
                    </table>
                </div>
                <input type="checkbox" class="vlr_tac" name="data[Relatorios][vlr_tac][]" value="1" />Considerar Valor TAC / Coeficiente Juros<br><br>
                <!-- <input type="checkbox" class="agrupar_filiais" name="data[Relatorios][agrupar_filiais][]" value="1" />Agrupar Filiais -->
                <strong>Agrupar Por</strong><br>
                <div class="container5">
                    <table id="tabelaPedPendente" >
                        <tr>
                            <td><input name="opt_agrupar" type="radio" value="condicao_pagamento"/></td>
                            <td style="vertical-align: middle;" >Condição Pagamento</td>
                        </tr>
                        <tr>
                            <td><input name="opt_agrupar" type="radio" value="tipo_pagamento" checked="checked"/></td>
                            <td style="vertical-align: middle;" >Tipo Pagamento</td>
                        </tr>
                    </table>
                </div>
            </td>
        ...
        <tr>
            <td colspan="2"><b>Tipos de Pagamento</b><br>
                <div class="container3">
                    <table id="tabela">
                        <tr>
                            <td style="width: 13px;"><input type="checkbox" id="marcaTodosPagamentos" value="" checked="true" /></td>
                            <td>TODOS TIPOS PAGAMENTOS</td>
                        </tr>
                        <?php foreach ($pagamentos as $pagamento) { ?>
                        <tr>
                            <td>
                                <input type="checkbox" class="pagamentos" name="data[Relatorios][pagamentos][]" value="<?php echo $pagamento["GlbTpPgto"]["cd_tipo_pgto"]; ?>" />
                            </td>
                            <td><?php echo utf8_encode($pagamento["GlbTpPgto"]["ds_tipo_pgto"]); ?></td>
                        </tr>
                        <?php } ?>
                    </table>
                </div>
