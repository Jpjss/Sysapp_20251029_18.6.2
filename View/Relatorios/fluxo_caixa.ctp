...
<script>
	...
	function filial(){
		...
		var checkedsFiliais = new Array();
		$("input[name='data[Relatorios][filial][]']:checked").each(function (){
			checkedsFiliais.push( $(this).val());
		});
    	...
    	if(checkedsFiliais != ""){
			...
			$.ajax({
			    type: "POST",
			    url: CbunnyObj.APP_PATH + 'Relatorios/carrega_checkbox_caixa',
			    data: {
			    	cd_filial : checkedsFiliais
			    	 },
			    dataType: "html",
			    success: function(data){
					$('#tabelaCaixa').html(data);
				    $.unblockUI();
			    }
			});
    	}else{
    		$('#tabelaCaixa').html('<tr><td style="width: 13px;"><input type="checkbox" id="marcaTodasCategorias" value="" onclick="javascript:marca()"/></td><td>TODAS AS CATEGORIAS</td></tr>');
        }
	}
    ...
    (function ($) {
    	    var currentYr = (new Date()).getFullYear(),
    	        ...
    	        dst;
    	    $('#RelatoriosFluxoCaixaForm').validate({
    	        rules: {
    	            dt_fi : 'dateITA',
    	            dt_in : 'dateITA',
    	        },
    	    ...
    	    $('#dt_fi').on('onChange', function () {
    	        $('#RelatoriosFluxoCaixaForm').valid();
    	    });
...
<div class="prevFinanceiraPagar">
    <h2><?php echo __('Relatório de Fluxo de Caixa'); ?></h2>
    <div class='mensagem'>
    </div>
    ...
    <table class="tabelaPrincipal">
        <tr>
            <td colspan="2"><b>Filiais</b><br>
                <div class="container3">
                    <table id="tabela">
                        <tr>
                            <td style="width: 13px;"><input class="filiais" type="checkbox" id="marcaTodasFilial" value="" onclick="javascript:filial()"/></td>
                            <td>TODAS FILIAIS</td>
                        </tr>
        ...
        <tr style="background-color: #FFFFFF">
            <td colspan="2"><b>Caixas</b><br>
                <div class="container3">
                    <table id="tabelaCaixa">
                        <tr>
                            <td style="width: 13px;"><input type="checkbox" id="marcaTodasCategorias" value="" /></td>
